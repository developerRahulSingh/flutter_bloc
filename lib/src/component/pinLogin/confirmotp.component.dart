...
class ConfirmOTPComponent extends StatefulWidget {
  ...
  const ConfirmOTPComponent(
      this.username, this.password, this.pinCode, this.sessionToken);

  @override
  _ConfirmOTPComponentState createState() => _ConfirmOTPComponentState();
}

class _ConfirmOTPComponentState extends State<ConfirmOTPComponent> {
...
//      }
  ...
  pinIndexSetUp(context, String enteredPin, String username, String password,
      String pinCode, String sessionToken) {
    if (pinIndex < 6) {
      if (pinIndex == 0)
        pinIndex = 1;
      ...
      if (pinIndex == 6) {
        ...
        if (strPin == pinCode) {
          SecurityUtil.setPINCodeSecurity(strPin);
          SecurityUtil.saveUserCredentials(username, password);
          //login method
          _signIn(context, username, password, sessionToken);
        } else {
          clearAllPin();
  ...
  buildNumberPad(context, String userName, String password, String pinCode,
      String sessionToken) {
    return Container(
      ...
      child: Wrap(
        ...
        children: <Widget>[
          KeyboardNumber(
            ...
            onPressed: () {
              pinIndexSetUp(
                  context, '1', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 2,
            onPressed: () {
              pinIndexSetUp(
                  context, '2', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 3,
            onPressed: () {
              pinIndexSetUp(
                  context, '3', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 4,
            onPressed: () {
              pinIndexSetUp(
                  context, '4', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 5,
            onPressed: () {
              pinIndexSetUp(
                  context, '5', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 6,
            onPressed: () {
              pinIndexSetUp(
                  context, '6', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 7,
            onPressed: () {
              pinIndexSetUp(
                  context, '7', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 8,
            onPressed: () {
              pinIndexSetUp(
                  context, '8', userName, password, pinCode, sessionToken);
            },
          ),
          KeyboardNumber(
            n: 9,
            onPressed: () {
              pinIndexSetUp(
                  context, '9', userName, password, pinCode, sessionToken);
            },
          ),
          Container(
            width: 64.0,
          ...
          KeyboardNumber(
            ...
            onPressed: () {
              pinIndexSetUp(
                  context, '0', userName, password, pinCode, sessionToken);
            },
          ),
          Container(
            width: 64.0,
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    buildNumberPad(context, widget.username, widget.password,
                        widget.pinCode, widget.sessionToken),
                    Padding(
